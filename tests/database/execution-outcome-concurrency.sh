...
insert into operations.operations(id,organisation_id,identity_id,idempotency_key,action,resource_id,purpose,expires_at) values ('$op','$org','$identity','outcome-concurrency','annotate','$resource','outcome concurrency',clock_timestamp()+interval '1 hour') on conflict (id) do nothing;
commit;
SQL
...
select set_config('request.jwt.claim.sub','$auth_user',true);
select set_config('app.organisation_id','$org',true);
select (recorded || '|' || decision) from operations.record_execution_outcome('$op',1,'acknowledged',null,'result://race-$n','hash-race-$n');
commit;
SQL
  ) &
done
