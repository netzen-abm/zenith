...
const outsiderRead = await psql(`select count(*)::text from core.evidence where source_id='${ids.source}' and resource_id='${ids.resource}';`);
const outsiderCount = outsiderRead.trim().split(/\\s+/).filter(Boolean).at(-1);
