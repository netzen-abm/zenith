...
The existing verification contract explicitly requires authorization negative tests, cross-tenant isolation, public-safe projection, sensitive-heritage coordinate policy, and auditability; passing unit tests alone is not an architecture gate pass.
...
Optional configuration may identify a dedicated test project, cleanup mode, or CI environment. Production credentials MUST NOT be used.
